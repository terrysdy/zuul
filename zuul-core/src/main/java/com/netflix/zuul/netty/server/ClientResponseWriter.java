 ...
 * 给 client 的 response 处理
 * <p>
 * 1. pipeline 里最后一个 inBound channel handler
 * 2. 在 outBound filter 处理完后 fireChannelRead 触发该 read() 方法处理
 * 3. read() 方法中将 response 写入 channel，将结果响应给 client，把 response 交给后续 outBound channel handler 处理（http
 * 报文、统计相关逻辑）
 */
public class ClientResponseWriter extends ChannelInboundHandlerAdapter {
    ...
    public ClientResponseWriter(RequestCompleteHandler requestCompleteHandler, Registry registry) {
        ...
        this.responseBeforeReceivedLastContentCounter = registry.counter(
                "server.http.requests.responseBeforeReceivedLastContent");
    }
    ...
    public void channelRead(final ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        if (msg instanceof HttpResponseMessage) {
            ...
            if ((!isHandlingRequest) || (startedSendingResponseToClient)) {
                /* This can happen if we are already in the process of streaming response back to
                 client OR NOT within active
                   request/response cycle and something like IDLE or Request Read timeout occurs.
                    In that case we have no way
                   to recover other than closing the socket and cleaning up resources used by
                   BOTH responses.
                 */
            ...
            if (channel.isActive()) {
                ...
                if (!ClientRequestReceiver.isLastContentReceivedForChannel(channel)) {
                    ...
                    if (ZuulStatusCategory.FAILURE_CLIENT_TIMEOUT.equals(status)) {
                        // If the request timed-out while being read, then there won't have been
                        // any LastContent, but thats ok because the connection will have to be
                        // discarded anyway.
                    } else {
                        responseBeforeReceivedLastContentCounter.increment();
                        LOG.warn(
                                "Writing response to client channel before have received the "
                                        + "LastContent of request! "
                ...
                // 将 outBound filter 处理后的响应给 client
    ...
    private static void writeBufferedBodyContent(final HttpResponseMessage zuulResponse,
    ...
    private HttpResponse buildHttpResponse(final HttpResponseMessage zuulResp) {
        ...
        // Now set all of the response headers - note this is a multi-set in keeping with HTTP
        // semantics
        ...
        // Netty does not automatically add Content-Length or Transfer-Encoding: chunked. So we
        // add here if missing.
        if (!HttpUtil.isContentLengthSet(nativeResponse) && !HttpUtil.isTransferEncodingChunked(
                nativeResponse)) {
            nativeResponse.headers()
                    .add(HttpHeaderNames.TRANSFER_ENCODING, HttpHeaderValues.CHUNKED);
        }

        final HttpRequest nativeReq = (HttpRequest) zuulResp.getContext()
                .get(CommonContextKeys.NETTY_HTTP_REQUEST);
        if (!closeConnection && HttpUtil.isKeepAlive(nativeReq)) {
            HttpUtil.setKeepAlive(nativeResponse, true);
        } else {
            // Send a Connection: close response header (only needed for HTTP/1.0 but no harm in
            // doing for 1.1 too).
            nativeResponse.headers().set("Connection", "close");
        }

        // TODO - temp hack for http/2 handling.
        if (nativeReq.headers()
                .contains(HttpConversionUtil.ExtensionHeaderNames.STREAM_ID.text())) {
            String streamId = nativeReq.headers()
        ...
        if (evt instanceof StartEvent) {
            ...
            if (reason == SESSION_COMPLETE || reason == INACTIVE) {
                ...
                if (isHandlingRequest) {
                    LOG.warn(
                            "Received complete event while still handling the request. With "
                                    + "reason: "
                                    + reason.name()
                                    + " -- "
                                    +
    ...
    private void handleComplete(Channel channel) {
        try {
            if ((isHandlingRequest)) {
                ...
                final HttpRequestMessage zuulRequest = ClientRequestReceiver.getRequestFromChannel(
                        channel);
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        ...
        final String errorMsg =
                "ClientResponseWriter caught exception in client connection pipeline: "
                        +
                        ChannelUtils.channelInfoForLogging(ctx.channel());
        ...
        if (isHandlingRequest && !startedSendingResponseToClient && ctx.channel().isActive()) {
            final HttpResponse httpResponse = new DefaultFullHttpResponse(HTTP_1_1,
